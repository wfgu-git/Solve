...
int LineEqual(Line l1, Line l2) {
  if (!Parallel(l1, l2))
    ...
    return (fabs(l1.a * l2.c - l2.a * l1.c) < eps &&
...
int IsLineIntersectSegment(Point p1, Point p2, Point s, Point e) {
  if (Cross(p1, p2, s) * Cross(p1, p2, e) > eps)
    return 0;
  else
    return 1;
}
...
int IsLineIntersectSegment(Line l1, Point s, Point e) {
  if (Cross(l1.p1, l1.p2, s) * Cross(l1.p1, l1.p2, e) > eps)
    return 0;
  else
    return 1;
}
...
Point BaryCenter(Point *p, int n) {
  ...
  for (i = 1; i < n - 1; i++) {
    ...
    s += t;
    res.x +=
        (p[0].x + p[i].x + p[i + 1].x) * t;  //将面积作为重量放在三角形的重心上
    res.y += (p[0].y + p[i].y + p[i + 1].y) *
             t;  //质量*坐标，三角形重心所需的除以3放在后面
  }
  res.x /= (3 * s);
  res.y /= (3 * s);
  return res;
...
void CircleCrossLine(Point a, Point b, Point o, double r, Point ret[],
                     int &num) {
...
double Calc(Point a, Point b, double r) {
  ...
  if (ina) {
    ...
    if (inb)
      return SectorArea(p[0], a, r) + fabs(Cross(p[0], b)) / 2.0;
    else {
      //情形4：两个点都在圆外，但是两点的连线与圆有两个焦点：分割成一个完全在圆内的三角形和两个扇形
      if (num == 2)
        return SectorArea(a, p[0], r) + SectorArea(p[1], b, r) +
...
int Graham(Point p[], int n, int res[]) {
  ...
  res[++top] = n - 2;
  for (i = n - 3; i >= 0; i--) {
    while (top != len && epssgn(Cross(p[i], p[res[top]], p[res[top - 1]])) >= 0)
      top--;
    res[++top] = i;
  }
  return top;
...
int PointInConvexHull(Point p[], int res[], int chnum, Point x) {
  ...
  while (l + 1 < r) {
    ...
    if (epssgn(Cross(p[res[l]] - g, p[res[mid]] - g)) > 0) {
      if (epssgn(Cross(p[res[l]] - g, x - g)) >= 0 &&
          epssgn(Cross(p[res[mid]] - g, x - g)) < 0)
        r = mid;
      else
        l = mid;
    } else {
  ...
  r %= chnum;
  if (epssgn(Cross(p[res[r]] - x, p[res[l]] - x)) == -1)
    return 1;
  else
    return 0;
}
...
double Diameter(Point p[], int res[], int chnum, int app[][2],
                int &appnum)  // AntiPodal Point
{
  int i, j;
  double ret = 0, nowlen;
  res[chnum] = res[0];
...
double ConvexHullMaxTriangleArea(Point p[], int res[], int chnum) {
  ...
  for (i = 0; i < chnum; i++) {
    ...
    while (fabs(Cross(p[res[j]] - p[res[i]],
                      p[res[(k + 1) % chnum]] - p[res[i]])) >
...
double TwoConvexHullMinDist(Point P[], Point Q[], int n, int m) {
  ...
  for (i = 0; i < n; i++) {
    ...
    while (tmp = Cross(Q[YMaxQ + 1] - P[YMinP + 1], P[YMinP] - P[YMinP + 1]) >
      ...
      YMaxQ = (YMaxQ + 1) % m;
    if (tmp < 0)
      ans = min(ans, PointToSegDist(P[YMinP], P[YMinP + 1], Q[YMaxQ]));
    else
      ans = min(ans,
                TwoSegMinDist(P[YMinP], P[YMinP + 1], Q[YMaxQ], Q[YMaxQ + 1]));
    YMinP = (YMinP + 1) % n;
  }
  return ans;
}
...
int cmp(const Line &l1, const Line &l2) {
  ...
  if (!d)
    return (epssgn(Cross(l2.p1 - l1.p1, l2.p2 - l1.p1)) >
            0);  //极角相同时，将更靠半平面里面的放在前面
  return d < 0;
}
...
void HalfPlaneIntersect(Line l[], int n, Point p[], int &pn) {
  ...
  //极角相同时，只保留最靠里面的那条
  for (i = j = 0; i < n; i++)
    if (epssgn(l[i].angle - l[j].angle) > 0) l[++j] = l[i];
  n = j + 1;
  dq[0] = 0;  //双端队列
  dq[1] = 1;
  top = 1;  //顶部和底部
  ...
  for (i = 2; i < n; i++) {
    ...
    while (top > bot &&
           IntersectionOutOfHalfPlane(l[i], l[dq[top]], l[dq[top - 1]]))
      top--;
    //由于求的是一个凸多边形，所以当半平面转过接近一圈时，某个半平面满足上一个while的条件后，
    //它又会影响到底部的两条直线，当底部的两条直线的交点，在当前的半平面外部时，底部弹栈
    while (top > bot &&
           IntersectionOutOfHalfPlane(l[i], l[dq[bot]], l[dq[bot + 1]]))
      bot++;
    dq[++top] = i;  //当前半平面入栈
  }
  //当最顶部的两条直线的交点不在最底部的半平面内时，顶部的那个半平面是多余的，顶部弹栈
  while (top > bot &&
         IntersectionOutOfHalfPlane(l[dq[bot]], l[dq[top]], l[dq[top - 1]]))
    top--;
  //当最底部的两条直线的交点不在最顶部的半平面内时，底部的那个半平面是多余的，底部弹栈
  while (top > bot &&
         IntersectionOutOfHalfPlane(l[dq[top]], l[dq[bot]], l[dq[bot + 1]]))
    bot++;
