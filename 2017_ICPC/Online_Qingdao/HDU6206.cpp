...
// void ArrayMUL(TInt32bit* result,long rminsize,const TInt32bit* x,long
// xsize,const TInt32bit* y,long ysize);//数组乘, 需要优化的首要目标
...
class TCatchIntError {
 private:
  ...
  inline SelfType& mul(const TInt& iValue) {
    if (iValue == 0)
      ...
      if ((iValue < MinValue) || (iValue > MaxValue) || (tmp < MinValue) ||
          (tmp > MaxValue) || ((tmp / iValue) != m_Int))
 ...
 public:
  ...
  inline TCatchIntError(const SelfType& Value) : m_Int(0) {
    (*this) += (Value.m_Int);
  }
  ...
  inline SelfType& operator+=(const SelfType& Value) {
    return (*this) += (Value.m_Int);
  }  // throw(TLargeFloatException)
  inline SelfType& operator-=(const SelfType& Value) {
    return (*this) -= (Value.m_Int);
  }  // throw(TLargeFloatException)
  inline SelfType& operator*=(const SelfType& Value) {
...
class TLargeFloat {
 public:
  ...
  enum {
    ...
    emLongDoubleDigits = std::numeric_limits<
        long double>::digits10,  // long double的10进制有效精度
    emLongDoubleMaxExponent = std::numeric_limits<
        long double>::max_exponent10,  // long double的最大10进制指数
  ...
  typedef TCatchIntError<TMaxInt, TException, TMaxInt_MIN_VALUE,
                         TMaxInt_MAX_VALUE>
      TExpInt;  //指数类型
 ...
 public:
  class
      TDigits  // TDigits用来设置TLargeFloat的精度;//增加这个类是为了避免TLargeFloat的构造函数的可能误用
  {
   private:
   ...
   public:
    ...
    inline const unsigned long GetDigitsArraySize() const {
      return m_DigitsArraySize;
    }
  };
  TLargeFloat(const SelfType& Value);
  explicit TLargeFloat(
      const long double
          DefultValue);  //默认浮点精度  //注意：转换可能存在小的误差
  explicit TLargeFloat(
      const long double DefultValue,
      const TDigits&
          DigitsLength);  // TDigits 十进制有效位数 //注意：转换可能存在小的误差
  ...
  inline void SetDigitsLength(const long uiDigitsLength) {
    SetDigitsLength(TDigits(uiDigitsLength));
  }
  ...
  inline void StrToLargeFloat(const std::string& strValue) {
    sToLargeFloat(strValue);
  }
  inline void StrToLargeFloat(const char* strValue) {
  ...
  SelfType& operator+=(const SelfType& Value);
  SelfType& operator-=(const SelfType& Value);
  inline SelfType& operator+=(long double fValue) {
    return (*this) += TLargeFloat(fValue);
  }
  inline SelfType& operator-=(long double fValue) {
    return (*this) -= TLargeFloat(fValue);
  }
  friend inline const TLargeFloat operator+(const TLargeFloat& x,
                                            const TLargeFloat& y) {
    TLargeFloat temp(x);
    return temp += y;
  }
  ...
  friend inline const TLargeFloat operator+(long double x,
                                            const TLargeFloat& y) {
    return y + x;
  }
  friend inline const TLargeFloat operator-(long double x,
  ...
  friend inline bool operator==(const TLargeFloat& x, const TLargeFloat& y) {
    return (x.Compare(y) == 0);
  }
  friend inline bool operator<(const TLargeFloat& x, const TLargeFloat& y) {
    return (x.Compare(y) < 0);
  }
  friend inline bool operator==(const TLargeFloat& x, long double y) {
    return (x == TLargeFloat(y));
  }
  friend inline bool operator<(const TLargeFloat& x, long double y) {
    return (x < TLargeFloat(y));
  }
  friend inline bool operator==(long double x, const TLargeFloat& y) {
    return (y == x);
  }
  friend inline bool operator<(long double x, const TLargeFloat& y) {
    return (y > x);
  ...
  friend inline void swap(TLargeFloat& a, TLargeFloat& b) {
    a.Swap(b);
  }  //交换值
  friend inline std::ostream& operator<<(std::ostream& cout,
                                         const TLargeFloat& Value) {
    return cout << Value.AsString();
  }
 ...
 private:
  ...
  TArray
      m_Digits;  //小数部分
                 //排列顺序是TArray[0]为第一个小数位(em10Power个10进制位),依此类推；取值范围[0--(emBase-1)]
 private:
  void Canonicity();  //规格化 转化值到合法格式
  void fToLargeFloat(
      long double fValue);  //内部使用 浮点数转化为 TLargeFloat,并采用默认精度
  void iToLargeFloat(
      TMaxInt iValue);  //内部使用 整数转化为 TLargeFloat,并采用默认精度
  void sToLargeFloat(
      const std::string& strValue);  //内部使用 字符串转化为 TLargeFloat
  long Compare(const SelfType& Value)
      const;  //比较两个数；(*this)>Value 返回1，小于返回-1，相等返回0
...
class TDigitsLengthCtrl {
 ...
 public:
  ...
  void SetDigitsLength(const long uiDigitsLength) {
    ...
    for (long i = 0; i < size; ++i) m_list[i]->SetDigitsLength(uiDigitsLength);
  }
};
...
class TDigitsLengthAutoCtrl : public TDigitsLengthCtrl {
 ...
 public:
  TDigitsLengthAutoCtrl(
      long DigitsMul, long DigitsLength0,
      long MaxDigitsLength)  //(收敛系数,初始精度,最终需要精度)
  {
...
void TLargeFloat_CtrlExponent(TLargeFloat::TArray& v, TMaxInt iMoveRightCount) {
  ...
  if (iMoveCountBig > 0) {
    ...
    for (long j = 0; j < iMoveCountBig; ++j) v[j] = 0;
  }
  //小的右移
...
void TLargeFloat_ArrayAdd(TInt32bit* result, long rsize, const TInt32bit* y,
                          long ysize) {
...
void TLargeFloat_ArraySub(TInt32bit* result, long rsize, const TInt32bit* y,
                          long ysize) {
  // MyDebugAssert(rsize>=ysize);
  // MyDebugAssert(ysize>0);
  TInt32bit* ri = &result[rsize - ysize];
...
TInt32bit _inti_csMaxMuliValue() {
  ...
  return (TInt32bit)(TInt32bit_MAX_VALUE * (1.0 / (TLargeFloat::emBase - 1)) /
                     rb) -
         1;
}
//不超界所允许的乘数值M满足: (emBase-1)*M + (emBase-1)*M/B + (emBase-1)*M/B^2 +
//(emBase-1)*M/B^3 ... <=TInt32bit_MAX_VALUE
static const TInt32bit csMaxMuliValue = _inti_csMaxMuliValue();
static const TInt32bit csMaxMulAddValue =
    csMaxMuliValue - (TLargeFloat::emBase - 1);

void TLargeFloat_ArrayMUL_ExE(TInt32bit* result, long rminsize,
                              const TInt32bit* x, long xsize,
                              const TInt32bit* y, long ysize) {
  // N*N复杂度的简单乘法实现
...
//数组乘法  核心
void ArrayMUL(TInt32bit* result, long rminsize, const TInt32bit* x, long xsize,
              const TInt32bit* y, long ysize);

void ArrayMUL_Dichotomy(TInt32bit* result, long rminsize, const TInt32bit* x,
                        const TInt32bit* y, long MulSize) {
  //二分法的乘法实现
  ...
  ArrayMUL(&result[abcdPos], std::min(rminsize - abcdPos, 2 * (E + 1)),
           temp_a_add_b, E + 1, temp_c_add_d, E + 1);
  ...
  if (sub_bd_size > 0) {
    ...
    if (add_bd_size > 0)
      TLargeFloat_ArrayAdd(result, add_bdPos + add_bd_size, temp_bd,
                           add_bd_size);
    TLargeFloat_ArraySub(result, sub_bdPos + sub_bd_size, temp_bd, sub_bd_size);
  }
}

void ArrayMUL_DichotomyPart(TInt32bit* result, long rminsize,
                            const TInt32bit* x, long xsize, const TInt32bit* y,
                            long ysize) {
  //二分法的辅助函数
  ///////////////////////////////////////////
...
TLargeFloat::TLargeFloat(const SelfType& Value) : m_Digits(Value.m_Digits) {
...
void TLargeFloat::Zero() {
  ...
  for (long i = 0; i < old_size; ++i) m_Digits[i] = 0;
}
...
void TLargeFloat::Canonicity() {
  ...
  while (m_Digits[0] >= emBase)  //是否需要右移1
  {
    ...
    for (long i = old_size - 1; i >= 2; --i) m_Digits[i] = m_Digits[i - 1];
  ...
  if (iMoveCountBig > 0) {
    ...
    for (long k = old_size - iMoveCountBig; k < old_size; ++k) m_Digits[k] = 0;
  }
  ...
  if (iMoveCountSmall > 0) {
    ...
    for (long i = old_size - 1; i >= 0; --i) m_Digits[i] *= iMoveMul;
...
void TLargeFloat::iToLargeFloat(TMaxInt iValue) {
  ...
  if ((long)m_Digits.size() < new_size) m_Digits.resize(new_size, 0);
  if (iValue == 0)
    return;
  else if (iValue > 0)
    m_Sign = 1;
  ...
  long n = 0;
  for (; tmp_iValue != 0; ++n) tmp_iValue /= emBase;
...
long double TLargeFloat::AsFloat() const {
  ...
  long double result =
      m_Sign * pow((long double)10.0, (long double)m_Exponent.AsInt());
...
inline void sToLargeFloat_setAChar(TLargeFloat::TArray& Digits, long set_index,
                                   char aChar) {
  unsigned long Value = aChar - '0';
  long e10Power_count =
      TLargeFloat::em10Power - 1 - (set_index % TLargeFloat::em10Power);
  for (long i = 0; i < e10Power_count; ++i) Value *= 10;
...
void TLargeFloat::sToLargeFloat(const std::string& strValue) {
  ...
  if ((int_end - int_begin >= 2) && ((*int_begin) == '0'))
    throw TException(
        "ERROR:TLargeFloat::sToLargeFloat(); 0");  //'0?'数字表示错误
  pstr_begin = int_end;
  if (((*pstr_begin) != '.') && (!is_have_int))
    throw TException("ERROR:TLargeFloat::sToLargeFloat(); 1");  //没有任何数字
  ...
  if ((*pstr_begin) == '.') {
    ...
    if ((!is_have_digits) && (!is_have_int))
      throw TException("ERROR:TLargeFloat::sToLargeFloat(); 2");  //没有任何数字
  ...
  if (((*pstr_begin) == 'e') || ((*pstr_begin) == 'E')) {
    ...
    if ((ep_int_end - ep_int_begin >= 2) && ((*ep_int_begin) == '0'))
      throw TException(
          "ERROR:TLargeFloat::sToLargeFloat(); 3");  //'0?'数字表示错误
    pstr_begin = ep_int_end;
    if (!is_have_ep_int)
      throw TException("ERROR:TLargeFloat::sToLargeFloat(); 4");  //指数没有数字
  }
  if (pstr_begin != pstr_end)
    throw TException("ERROR:TLargeFloat::sToLargeFloat(); 5");  //未预料的字符
  ...
  else  //(m_Exponent==Value.m_Exponent)
  {
    ...
    if (sizeS > sizeV) {
      for (long i = sizeV; i < sizeS; ++i) {
        if (m_Digits[i] > 0) return m_Sign;
        ;
      }
    } else if (sizeS < sizeV) {
      for (long i = sizeS; i < sizeV; ++i) {
        if (Value.m_Digits[i] > 0) return -m_Sign;
      }
    }
    // sizeS==sizeV
...
void TLargeFloat::Abs_Add(const SelfType& Value)  //绝对值加法
{
  ...
  //被加数指数较大就可以了
  if (m_Exponent.AsInt() < Right.m_Exponent.AsInt()) this->Swap(Right);
  long size = m_Digits.size();
  ...
  if (m_Digits[0] >= emBase) Canonicity();
}
...
void TLargeFloat::Abs_Sub_Abs(const SelfType& Value)  //绝对值减法
{
  ...
  if (m_Digits[0] * 10 < emBase) Canonicity();
}
...
void TLargeFloat::DivInt(TMaxInt iValue)  //除以一个整数
{
  ...
  if (iValue <= csMaxDiviValue) {
    ...
    if (size >= 1) m_Digits[size - 1] /= iDiv;
  } else {
    TMaxInt iDiv = iValue;
    long size = m_Digits.size();
    ...
    if (size >= 1) m_Digits[size - 1] = (TInt32bit)(cur_value / iDiv);
  }
...
void TLargeFloat::Rev()  //求倒数
{
  ...
  TDigitsLengthAutoCtrl dlCtrl(2, emLongDoubleDigits - 1,
                               (a.m_Digits.size() * TLargeFloat::em10Power));
...
inline void getBestMulSize(const TInt32bit*& x, long& xsize,
                           const TInt32bit*& y, long& ysize) {
  long bxsize = xsize;
  long bysize = ysize;
  ...
  if ((bysize < csABigMulSize) || (bxsize * 2 >= bysize * 3) ||
      (bxsize > ysize))
    ysize = bysize;
  else
    ysize = bxsize;
...
TLargeFloat::SelfType& TLargeFloat::operator*=(const SelfType& Value) {
  ...
  if ((m_Sign == 0) || (Value.m_Sign == 0)) {
    if (m_Sign != 0) this->Zero();
    if (xsize < ysize) m_Digits.resize(ysize, 0);
    return *this;
  }
  if (xsize < ysize) m_Digits.resize(ysize, 0);
  ...
  if (rminsize > xsize + ysize) rminsize = xsize + ysize;
...
void InvFFT(TComplex* a, const TComplex* w, const long n) {
  for (long i = 0; i < n; ++i) a[i].i = -a[i].i;
  FFT(a, w, n);
}
...
void FFT_IntArrayCopyToComplexArray(const TInt32bit* ACoef, const long ASize,
                                    TComplex* result, const long n,
                                    const long* lst) {
  for (long i = 0; i < n; ++i) {
    long ai = lst[i];
    if (ai < ASize)
...
void FFT_ComplexArrayToIntArray(const TComplex* cmx, const long n,
                                TInt32bit* result, const long rMinSize) {
  ...
  for (long i = std::min(n, dstsize) - 1; i >= 1; --i) {
    ...
    if (newWorstEr > MaxWorstEr) MaxWorstEr = newWorstEr;
  }
  if (dstsize > 0) dst[0] = (TInt32bit)(tmp + 0.1);
  if (MaxWorstEr >= 0.499)
    throw TLargeFloat::TException(
        "ERROR:FFT's FFT_ComplexArrayToIntArray(); ");  //误差过大
}

// FFT平方
void SquareFFT(const TInt32bit* ACoef, const long ASize, TInt32bit* CCoef,
               const long CMinSize) {
...
// FFT乘法
void MulFFT(const TInt32bit* ACoef, const long ASize, const TInt32bit* BCoef,
            const long BSize, TInt32bit* CCoef, const long CMinSize) {
...
void LargeFloat_UnitTest() {
  ...
  TLargeFloat z("99999999999999999999999999999999999999999999",
                TLargeFloat::TDigits(200));
  //
}
...
int main() {
  ...
  for (int cas = 1; cas <= T; ++cas) {
    ...
    in(x3); in(y3);
    in(x); in(y);
    TLargeFloat X1(x1, TLargeFloat::TDigits(1000));
    TLargeFloat Y1(y1, TLargeFloat::TDigits(1000));
    TLargeFloat X2(x2, TLargeFloat::TDigits(1000));
    TLargeFloat Y2(y2, TLargeFloat::TDigits(1000));
    TLargeFloat X3(x3, TLargeFloat::TDigits(1000));
    TLargeFloat Y3(y3, TLargeFloat::TDigits(1000));
    TLargeFloat X(x, TLargeFloat::TDigits(1000));
    TLargeFloat Y(y, TLargeFloat::TDigits(1000));
    TLargeFloat a(X1 - X2);
    TLargeFloat b(Y1 - Y2);
    TLargeFloat c(X1 - X3);
    TLargeFloat d(Y1 - Y3);
    ...
    if ((X - X0) * (X - X0) + (Y - Y0) * (Y - Y0) > R_f) {
      out_str("Accepted"); out_char('\n');
    } else {
      out_str("Rejected"); out_char('\n');
    }
  }
  write();
  return 0;
